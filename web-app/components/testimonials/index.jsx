...
function Testimonials(props) {
  const { data: propsData, className } = props;
  const data = propsData || testimonialData;
                ...
                {data.map((item) => (
                  <SwiperSlide key={item.id}>
                    <div className="wd-testimonials">
                      <p className="description">{item.review || item.text}</p>
                      <div className="author-group">
                        <div className="avatar">
                          <img src={item.image || item.avt} alt={item.name} />
                        </div>
                        <div className="infor">
                          <h6>{item.name}</h6>
                          <div className="position">{item.position || item.postion}</div>
                          <ul className="rating">
